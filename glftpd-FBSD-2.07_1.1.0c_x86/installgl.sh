...
proc_createcert() {
  ...
  if [ $type = "dsa" ]; then
    ...
    $opensslbin ecparam -out $base.key -name secp521r1 -genkey 2>&1
  fi
...
if [ "$netd" = "i" ]; then
    ...
    { grep -v ^${servicename} /etc/inetd.conf
        echo "$servicename stream tcp nowait root $tcpd $jaildir$glroot/bin/glftpd $dummyflag -l -o -i $confpath $glstrings $euroweek" | tr -s ' '
    } > /etc/inetd.conf.new
